...
Like "standard" "compiled" reverse mode AD frameworks. Special care is take to capture as few things as possible, to keep the "tape" small.
...
timings = mapreduce(hcat, BATCH_TYPES) do BATCH_TYPE
    ...
    vcat(BATCH_TYPE, ctime1, ctime2, ctime3, btimes)
end

df = DataFrame(round2.(timings), :auto)
insertcols!(df, 1, :WHAT=>["BATCH_TYPE", "Prepare my gradient", "Prepare Mooncake gradient", "Prepare Enzyme gradient", "Plain evaluation", "My gradient", "Mooncake gradient", "Enzyme gradient"])
```